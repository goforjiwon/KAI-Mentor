...
-- 수업 기록 (서버 API를 통한 강사 본인 조회 및 관리자 관리)
...
create index if not exists lesson_reports_mentor_date_idx on public.lesson_reports (mentor_id, lesson_date desc, created_at desc);
create index if not exists lesson_reports_date_idx on public.lesson_reports (lesson_date desc, created_at desc);
create index if not exists lesson_reports_status_idx on public.lesson_reports (status, lesson_date desc);
...
-- 강사·학생·학부모 매칭 이력 (관리자 전용)
create table if not exists public.tutoring_matches (
  id uuid primary key default gen_random_uuid(),
  created_at timestamptz not null default now(),
...
create index if not exists tutoring_matches_created_idx on public.tutoring_matches(created_at desc,id);
create index if not exists tutoring_matches_mentor_idx on public.tutoring_matches(mentor_id,created_at desc);
create index if not exists tutoring_matches_application_idx on public.tutoring_matches(application_id,created_at desc);
create index if not exists tutoring_matches_status_idx on public.tutoring_matches(status,created_at desc);
create unique index if not exists tutoring_matches_open_uidx on public.tutoring_matches(mentor_id,application_id,lower(btrim(student_name))) where status in ('active','paused');
