...
export function MarketingHeader({ teacher = false }: { teacher?: boolean }) {
  return <header className="border-b border-slate-200 bg-white"><div className="mx-auto flex min-h-20 max-w-[1440px] items-center justify-between gap-4 px-5 py-4 sm:px-8 lg:px-16">
    <Link href="/" className="shrink-0 text-2xl font-bold tracking-[-0.06em] text-blue-600 sm:text-3xl">카이멘토</Link>
    <nav aria-label="주 메뉴" className="hidden items-center gap-8 text-sm font-semibold text-slate-700 md:flex">{teacher ? <><Link href="/mentor-signup">선생님 지원</Link><a href="#process">이용 방법</a></> : <><a href="#process">이용 방법</a><a href="#care">정기 상담</a><Link href="/teacher-guide">선생님 안내</Link></>}</nav>
