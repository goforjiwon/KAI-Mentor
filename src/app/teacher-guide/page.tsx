...
export default function TeacherGuidePage() {
  return <div className="min-h-screen bg-white text-slate-950"><MarketingHeader teacher /><main>
    <section><div className="mx-auto grid max-w-[1440px] lg:grid-cols-2 lg:items-center">
      ...
      <div className="min-w-0 bg-stone-50">
        <Image src="/images/teacher-preparation-hero.png" alt="수업을 준비하며 학습 자료를 정리하는 선생님" width={1448} height={1086} priority sizes="(max-width: 1023px) 100vw, (max-width: 1440px) 50vw, 720px" className="block h-auto w-full" />
        <div className="flex items-start gap-3 px-5 py-5 sm:px-8"><span className="shrink-0 text-emerald-700"><MarketingIcon kind="chat" /></span><div><p className="font-bold">수업료 안내는 카이멘토에서</p><p className="mt-2 text-sm leading-6 text-slate-600">선생님 대신 학부모님께 안내드립니다.</p></div></div>
      </div>
    </div></section>
    ...
    <section id="process" className="scroll-mt-6 px-5 py-12 sm:px-8"><div className="mx-auto max-w-7xl">
      ...
      <div className="mt-8 grid gap-5 md:grid-cols-2"><article className="flex flex-col items-start gap-4 rounded-2xl border border-slate-200 p-6 sm:p-8 lg:flex-row"><span className="shrink-0 text-emerald-800"><MarketingIcon kind="people" className="h-10 w-10" /></span><div><p className="text-sm font-bold text-emerald-700">선생님</p><h3 className="mt-2 text-xl font-bold">수업과 학습 피드백</h3><p className="mt-3 leading-7 text-slate-600">학생에게 맞는 수업을 진행하고 학습 상황을 카이멘토에 공유해 주세요.</p></div></article><article className="flex flex-col items-start gap-4 rounded-2xl border border-slate-200 p-6 sm:p-8 lg:flex-row"><span className="shrink-0 text-emerald-800"><MarketingIcon kind="chat" className="h-10 w-10" /></span><div><p className="text-sm font-bold text-emerald-700">카이멘토</p><h3 className="mt-2 text-xl font-bold">수납 안내와 학부모님 상담</h3><p className="mt-3 leading-7 text-slate-600">수업료 안내·입금 확인과 월 1회 학부모님 전화 상담을 맡습니다.</p></div></article></div>
    </div></section>
