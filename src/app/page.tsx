...
export default function HomePage() {
  return <div className="min-h-screen bg-white text-slate-950"><MarketingHeader /><main>
    <section className="border-b border-slate-100"><div className="mx-auto grid max-w-[1440px] lg:grid-cols-[minmax(0,47fr)_minmax(0,53fr)] lg:items-center">
      <div className="flex flex-col justify-center min-w-0 px-5 py-8 sm:px-8 sm:py-12 lg:px-10 xl:px-16">
      ...
      <div className="min-w-0 bg-slate-50">
        <Image src="/images/mentor-study-hero.png" alt="선생님과 학생이 함께 수학을 공부하는 모습" width={1672} height={941} priority sizes="(max-width: 1023px) 100vw, (max-width: 1440px) 53vw, 764px" className="block h-auto w-full" />
        <div className="flex items-start gap-3 px-5 py-5 sm:px-8"><span className="shrink-0 text-blue-600"><MarketingIcon /></span><div><p className="text-sm font-bold sm:text-base">학부모님 상담은 <span className="text-blue-600">카이멘토에서</span></p><p className="mt-2 text-xs leading-5 text-slate-600">월 1회, 수업 이후까지 함께 살펴드립니다.</p></div></div>
        <p className="border-t border-slate-200 px-5 py-3 text-xs text-slate-600 sm:px-8">KAIST 선생님과 1:1 수업</p>
      </div>
    </div></section>
    <section id="care" className="scroll-mt-6 bg-blue-50 px-5 py-12 sm:px-8 sm:py-14"><div className="mx-auto max-w-7xl">
      ...
      <div className="mt-8 grid gap-5 md:grid-cols-2">
        <article className="flex flex-col items-start gap-4 rounded-2xl bg-white p-6 sm:p-8 xl:flex-row xl:items-center"><span className="shrink-0 rounded-full bg-blue-50 p-4 text-blue-600 sm:p-6"><MarketingIcon kind="book" /></span><div><p className="text-sm font-bold text-blue-600">KAIST 선생님</p><h3 className="mt-2 text-xl font-bold">자녀에게 맞는 1:1 수업</h3><p className="mt-3 leading-7 text-slate-600">수학·과학 개념부터 문제 풀이까지 학생의 수준과 목표에 맞춰 지도합니다.</p></div></article>
        <article className="flex flex-col items-start gap-4 rounded-2xl border border-blue-600 bg-white p-6 sm:p-8 xl:flex-row xl:items-center"><span className="shrink-0 rounded-full bg-blue-50 p-4 text-blue-600 sm:p-6"><MarketingIcon /></span><div><p className="text-sm font-bold text-blue-600">카이멘토 정기 관리</p><h3 className="mt-2 text-xl font-bold">월 1회 학부모님 전화 상담</h3><p className="mt-3 leading-7 text-slate-600">수업 상황과 학부모님의 고민을 듣고 필요한 조정 사항을 선생님과 함께 정리합니다.</p></div></article>
      </div>
    </div></section>
    <section id="process" className="scroll-mt-6 px-5 py-12 sm:px-8 sm:py-14"><div className="mx-auto max-w-7xl">
      ...
      <ol className="mt-8 grid gap-5 lg:grid-cols-3">{careSteps.map(([title, body], index) => <li key={title} className="flex min-w-0 gap-4 rounded-2xl border border-slate-200 p-6"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-blue-50 font-bold text-blue-600">0{index + 1}</span><div><h3 className="font-bold">{title}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{body}</p></div></li>)}</ol>
